    ...
    <div class="add-plan zy-main-wraps">
        ...
        <div class="add">
            ...
            <ul v-for="salary in addplanOptions" class="salary-item">
                ...
                <li class="item-btn">
                    <el-button icon="el-icon-plus" @click="onAddItem(salary.itemType,salary.items)">添加薪资项</el-button>
                </li>
            </ul>
            <div class="footer-btn">
                <el-button class="button default-button" @click="cancalAdd">取消</el-button>
        ...
        <!--取消添加-->
        <el-dialog class="zy-dialog-pub zy-dialog-pub-medium" title="取消编辑薪资项" :visible.sync="isShowCancelEdit">
            <p class="word-explain">确认取消编辑此方案吗？</p>
            <div slot="footer" class="dialog-footer t-c">
                <el-button class="button default-button" @click="isShowCancelEdit = false">取 消</el-button>
        ...
        methods:{
            getData(){
                ...
                this.$http.get('/salaryPlan/querySalaryPlan',{
                    ...
                    this.planInfo = contents;
                    if(this.planInfo.workDay) this.salaryTimeType=1;
                    else this.salaryTimeType=2;

                    this.planInfo.planOptions.forEach(option=>{
                        option.status = option.status==1;
                        this.addplanOptions.find(f=> f.itemType == option.optionType).items.push(option);

                    });
                    // this.addplanOptions
                    console.log(this.planInfo);
                    this.getOption(this.planInfo.planOptions);
                })
            },
            getOption(selecteds){
                const params = {
                ...
                this.$http.get('/salaryPlan/getOptionList',{
                    ...
                    selecteds.forEach(selected=>{
                        const curOption= optionList.find(f=>f.optionId== selected.optionId);
                        if(curOption) curOption.status=1;
                    });
            ...
            getMonthList(){
                for (let i = 1;i<=12;i++){
                    this.monthList.push({month:`${i}月`,value:i})
                }
            },
            //弹框确认添加
            handleAddMoreItem(){
                this.itemnameList.forEach((item,index)=>{
                    if(item.planname !== ''){
                        ...
                        this.addplanOptions[this.dialogAddType].items.push(params)
                    }
                })
            ...
            setCalcRules(item,$index){
                ...
                // resource: ""
                this.setCalcRule ={
                    baseOptions:item.salaryRecord.baseOptions,
                    computeMode: item.salaryRecord.computeMode+'',
                    desc:'',
                    money: item.salaryRecord.computeMode==1 ? '': item.salaryRecord.computeValue,
                    monthList : this.monthList,
                    percent:item.salaryRecord.computeMode==1 ? item.salaryRecord.computeValue: '',
                    recordMonth:item.salaryRecord.recordMonth,
                    resource:''
                };
                // console.log(this.setCalcRule);
                // baseOptions: Array(3)
                // companyId: "1"
                // computeMode: 1
                // computeValue: 122
                // id: "29"
                // optionId: "190710000001810"
                // planId: "190711000001991"
                // recordId: "190711000001990"
                // recordMonth: Array(2)
                // recordName: "津贴补助1"
                // status: 0
                // type: 1
                // console.log(this.settingCalcItem.salaryRecord);
            ...
            handleSetCalc(){
                this.$refs['setCalcRule'].validate((valid) => {
                    if (valid) {
                        ...
                        this.isShowCalcRules=false;
                        // this.settingCalcItem.salaryRecord= params;
                    } else {
            ...
            // 添加薪资项
            onAddItem(optionType,items){
                console.log(items,this.addplanOptions[optionType-1].allItems);
                ...
                this.currentItemList = this.addplanOptions[optionType-1].allItems.map(data=>{
                    data.status = Boolean((items.find(f=> f.optionId == data.optionId)||{}).status);
            ...
            handleSaveAdd(){
                const haveSameitem = this.currentItemList.some(item=>{
                    return item.optionName.indexOf(this.planname)>-1;
                })
                if(haveSameitem){
            ...
            onConfirmAddItem(){
                ...
                this.addplanOptions[this.addDialogType-1].items = JSON.parse(JSON.stringify(curPickItemlist));
            ...
            confirmAdd(){
                ...
                this.planInfo.planOptions= options;
                let params = this.planInfo;
                console.log(params);
                // return;
                let that = this;
                this.$http.post('/salaryPlan/updateSalaryPlan',params).then(t=>{
                    console.log(t);
                    this.$message({
                        message: '修改方案成功',
                        type: 'success',
